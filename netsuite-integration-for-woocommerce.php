...
// function deactivate_events_on_active_plugin() {
...
function deactivate_events( $hook) {
	...
	foreach ( $crons as $timestamp => $cron ) {
		if ( ! empty( $cron['wc-checkout-process-start'] ) ) {
			unset( $crons[$timestamp]['wc-checkout-process-start'] );
		}
	}
	_set_cron_array( $crons );
}
