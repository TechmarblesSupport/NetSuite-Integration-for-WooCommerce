...
class OrderRefund extends CommonIntegrationFunctions {
				...
				if (isset($searchResponse->searchResult->status->isSuccess) && 1 == $searchResponse->searchResult->status->isSuccess) {
					...
					foreach ($records as $key => $record) {
						...
						if(isset($order_internalIds_array[$order_internal_id])){
							...
							$items = self::getRefundItem($order_refund_id);
							if(!empty($items)){
								// if($order_internalIds_array[$order_internal_id] != '2103'){
								// 	continue;
								// }

								self::orderRefundWoo($items,$order_internalIds_array[$order_internal_id],$order_refund_id);
	...
	public static function getRefundItem($order_refund_id){
		$ns_service = new NetSuiteService();
		...
		try{
			...
			if(isset($getResponse->readResponse->status->isSuccess) && $getResponse->readResponse->status->isSuccess == 1 ){
				if(isset($getResponse->readResponse->record->itemList->item)){
	...
	public static function orderRefundWoo($items,$order_id,$order_refund_id){
			...
			foreach ($woo_order_items as $woo_order_item_id => $woo_order_item ) {
				...
				if (isset($items[$product_id])) {
					...
					if($item_qty_refunded != $item->quantity){
						...
						$refund_amount = wc_format_decimal( $refund_amount ) + wc_format_decimal( $item_total + $refund_tax );
